...
Es existieren noch keine verlässlichen strukturierten Prefix-/Suffixkapazitäten. Der Resolver rät daher keine Grenzwerte: Ohne explizite `capacityRules` bleiben sonst zulässige Prefix- und Suffixkandidaten `unresolved`. Übergebene Regeln sind ausschließlich explizite Caller-Eingaben; sie werden nur ausgewertet und weder als Standard noch dauerhaft als Engine-Wahrheit gespeichert. Volle Prefix- und Suffixseiten werden unabhängig voneinander geprüft. `technicalTier` bleibt unverändert; `displayTier` wird separat und ausschließlich für die technische Itemklasse aufgelöst.
...
Der **Mutation Plan** beschreibt mit `applied: false` ausschließlich beabsichtigte Schritte wie `set-rarity`, `preserve-existing-modifiers`, `clear-random-modifiers`, `add-selected-modifier` oder `replace-random-modifiers`. Er mutiert den Item State nicht und gibt keinen scheinbar ausgeführten Folgezustand zurück. Transmutation plant normal→magic und eine caller-belegte Anzahl regulärer Mods; Augmentation erhält ein Magic Item und plant genau eine Addition; Regal plant magic→rare plus Addition; Exalted plant eine Addition auf Rare.
