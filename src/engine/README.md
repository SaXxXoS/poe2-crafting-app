...
Der **Mutation Plan** beschreibt mit `applied: false` ausschließlich beabsichtigte Schritte wie `set-rarity`, `preserve-existing-modifiers`, `clear-random-modifiers`, `add-selected-modifier` oder `replace-random-modifiers`. Er mutiert den Item State nicht und gibt keinen scheinbar ausgeführten Folgezustand zurück. Transmutation ist ausschließlich für ein leeres Normal-Item anwendbar, plant normal→magic und fügt mit dem autoritativen `selectionCount: 1` exakt ein reguläres Prefix oder Suffix hinzu; caller-gelieferte Count-Overrides bestimmen diese Anzahl nicht. Die bestehende Magic-Capacity muss ausdrücklich bereitgestellt werden, ohne eine Normal-Capacity einzuführen oder anzunehmen. Nicht erfolgreiche Resultate bleiben ohne partiellen Ergebnis-State, Rarity-Wechsel, Revisionserhöhung oder hinzugefügten Modifier. Augmentation erhält ein Magic Item und plant genau eine Addition; Regal plant magic→rare plus Addition; Exalted plant eine Addition auf Rare.
